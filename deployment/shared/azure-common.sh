...
confirm_azure_subscription() {
 print_info "Current Azure subscription:"az account show --query "{Name:name, SubscriptionId:id}"--output table

 read -p "Continue with this subscription? (y/N): "-n 1 -r
 echo
 if [[ ! $REPLY =~ ^[Yy]$ ]]; then
 print_warning "Deployment cancelled."exit 1
 fi
}

# Select resource group interactively
select_resource_group() {
 print_info "Available resource groups:"az group list --query "[].{Name:name, Location:location}"--output table
 ...
 print_info "Validating resource group: $RESOURCE_GROUP"LOCATION=$(az group show --name "$RESOURCE_GROUP"--query "location"--output tsv 2>/dev/null)

 if [ -z "$LOCATION"]; then
 print_error "Resource group '$RESOURCE_GROUP'not found."echo "Please make sure the resource group exists and you have access to it."exit 1
 fi
...
get_sql_password() {
 ...
 print_error "Passwords do not match. Please try again."fi
 done
...
validate_arm_template() {
 ...
 print_info "Validating ARM template..."if [ ! -f "$template_file"]; then
 print_error "Template file not found: $template_file"exit 1
 fi
 ...
 if [ $? -eq 0 ]; then
 print_success "Template validation successful"else
 print_error "Template validation failed. Please check the template."exit 1
 fi
}
...
deploy_arm_template() {
 ...
 print_info "Deploying ARM template..."az deployment group create \
 --resource-group "$resource_group"\
 ...
 if [ $? -eq 0 ]; then
 print_success "ARM template deployment completed"else
 print_error "ARM template deployment failed"exit 1
 fi
}
...
check_prerequisites() {
 ...
 print_error "Missing prerequisites:"for tool in "${missing_tools[@]}"; do
 echo "- $tool"done
 exit 1
 fi
